...
from fastapi import APIRouter, Depends, HTTPException  # Ensure APIRouter is correctly imported
...
async def create_user(user: UserCreateDTO, user_controller: UserController = Depends(get_user_controller)) -> UserDTO:
    return user_controller.create_user(user)

# Lister tous les utilisateurs
@router.get("/all", response_model=List[UserDTO])
def list_users(user_controller: UserController = Depends(get_user_controller)) -> List[UserDTO]:
    
    return user_controller.list_users()

# Obtenir un utilisateur par ID
@router.get("/{user_id}", response_model=UserDTO)
def get_user(user_id: str, user_controller: UserController = Depends(get_user_controller)) -> UserDTO:
...
def get_user_with_terms(user_id: str, user_controller: UserController = Depends(get_user_controller))-> UserWithTermsDTO:
